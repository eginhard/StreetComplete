...
@Singleton class QuestStatisticsDao @Inject constructor(private val db: Database) {
    ...
    suspend fun replaceAll(amounts: Map<String, Int>) {
        db.transaction {
            db.delete(NAME)
    ...
    suspend fun addOne(questType: String) {
        db.transaction {
    ...
    suspend fun subtractOne(questType: String) {
        db.exec("UPDATE $NAME SET $SUCCEEDED = $SUCCEEDED - 1 WHERE $QUEST_TYPE = ?", arrayOf(questType))
        listeners.forEach { it.onSubtractedOne(questType) }
    }

    suspend fun getAmount(questType: String): Int =
        db.queryOne(NAME,
    ...
    suspend fun getAmount(questTypes: List<String>): Int {
