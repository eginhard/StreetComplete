...
class ElementGeometryDao @Inject constructor(
    ...
    suspend fun put(entry: ElementGeometryEntry) {
        db.replace(NAME, entry.toPairs())
    }

    suspend fun get(type: Element.Type, id: Long): ElementGeometry? =
        db.queryOne(NAME,
            where = "$ELEMENT_TYPE = ? AND $ELEMENT_ID = ?",
            args = arrayOf(type.name, id)
        ) { it.toElementGeometry() }

    suspend fun delete(type: Element.Type, id: Long): Boolean =
        db.delete(NAME,
            where = "$ELEMENT_TYPE = ? AND $ELEMENT_ID = ?",
            args = arrayOf(type.name, id)
        ) == 1

    suspend fun putAll(entries: Collection<ElementGeometryEntry>) {
    ...
    suspend fun getAllKeys(bbox: BoundingBox): List<ElementKey> =
        db.query(NAME,
            columns = arrayOf(ELEMENT_TYPE, ELEMENT_ID),
            where = inBoundsSql(bbox)
        ) { it.toElementKey() }

    suspend fun getAllEntries(bbox: BoundingBox): List<ElementGeometryEntry> =
        db.query(NAME, where = inBoundsSql(bbox)) { it.toElementGeometryEntry() }

    suspend fun getAllEntries(keys: Collection<ElementKey>): List<ElementGeometryEntry> {
    ...
    suspend fun deleteAll(entries: Collection<ElementKey>):Int {
