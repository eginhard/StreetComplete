...
class RelationDao @Inject constructor(
    ...
    suspend fun put(relation: Relation) {
        putAll(listOf(relation))
    }

    suspend fun get(id: Long): Relation? =
        getAll(listOf(id)).firstOrNull()

    suspend fun delete(id: Long): Boolean =
        deleteAll(listOf(id)) == 1

    suspend fun putAll(relations: Collection<Relation>) {
    ...
    suspend fun getAll(ids: Collection<Long>): List<Relation> {
    ...
    suspend fun deleteAll(ids: Collection<Long>): Int {
    ...
    suspend fun getAllForNode(nodeId: Long) : List<Relation> =
        getAllForElement(Element.Type.NODE, nodeId)

    suspend fun getAllForWay(wayId: Long) : List<Relation> =
        getAllForElement(Element.Type.WAY, wayId)

    suspend fun getAllForRelation(relationId: Long) : List<Relation> =
        getAllForElement(Element.Type.RELATION, relationId)

    suspend fun getIdsOlderThan(timestamp: Long): List<Long> =
        db.query(NAME, columns = arrayOf(ID), where = "$LAST_UPDATE < $timestamp") { it.getLong(ID) }

    private suspend fun getAllForElement(elementType: Element.Type, elementId: Long): List<Relation> {
        val ids = db.query(
            NAME_MEMBERS,
            columns = arrayOf(ID),
