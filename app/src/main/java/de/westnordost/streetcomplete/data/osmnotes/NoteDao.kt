...
class NoteDao @Inject constructor(
    ...
    suspend fun putAll(notes: Collection<Note>) {
    ...
    suspend fun getAll(bbox: BoundingBox): List<Note> =
    ...
    suspend fun getAll(ids: Collection<Long>): List<Note> {
        if (ids.isEmpty()) return emptyList()
        return db.query(NAME, where = "$ID IN (${ids.joinToString(",")})") { it.toNote() }
    }

    suspend fun getAllIdsOlderThan(timestamp: Long): List<Long> =
