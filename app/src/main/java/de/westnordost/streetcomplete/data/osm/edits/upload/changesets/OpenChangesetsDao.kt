...
class OpenChangesetsDao @Inject constructor(private val db: Database) {
    ...
    suspend fun getAll(): Collection<OpenChangeset> =
        db.query(NAME) { it.toOpenChangeset() }

    suspend fun put(openChangeset: OpenChangeset) {
        db.replace(NAME, openChangeset.toPairs())
    }

    suspend fun get(questType: String, source: String): OpenChangeset? =
        db.queryOne(NAME,
            where = "$QUEST_TYPE = ? AND $SOURCE = ?",
            args = arrayOf(questType, source)
        ) { it.toOpenChangeset()  }

    suspend fun delete(questType: String, source: String): Boolean =
        db.delete(NAME,
            where = "$QUEST_TYPE = ? AND $SOURCE = ?",
            args = arrayOf(questType, source)
        ) == 1
