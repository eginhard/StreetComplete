...
@Singleton class CountryStatisticsDao @Inject constructor(private val db: Database) {
    ...
    suspend fun getCountryWithBiggestSolvedCount(): CountryStatistics? =
        db.queryOne(NAME, orderBy = "$SUCCEEDED DESC") { it.toCountryStatistics() }

    suspend fun getAll(): List<CountryStatistics> =
    ...
    suspend fun replaceAll(countriesStatistics: Collection<CountryStatistics>) {
        db.transaction {
            db.delete(NAME)
    ...
    suspend fun addOne(countryCode: String) {
        db.transaction {
    ...
    suspend fun subtractOne(countryCode: String) {
        db.exec("UPDATE $NAME SET $SUCCEEDED = $SUCCEEDED - 1 WHERE $COUNTRY_CODE = ?", arrayOf(countryCode))
    }
}
