...
class OsmQuestDao @Inject constructor(private val db: Database) {
    ...
    suspend fun add(quest: OsmQuestDaoEntry): Boolean {
    ...
    suspend fun get(id: Long): OsmQuestDaoEntry? =
        db.queryOne(NAME, null, "$QUEST_ID = $id") { it.toOsmQuestEntry() }

    suspend fun getAllInBBoxCount(bounds: BoundingBox): Int {
    ...
    suspend fun addAll(quests: Collection<OsmQuestDaoEntry>) {
    ...
    suspend fun getAllForElement(elementType: Element.Type, elementId: Long): List<OsmQuestDaoEntry> =
        db.query(NAME,
            where = "$ELEMENT_TYPE = ? AND $ELEMENT_ID = ?",
            args = arrayOf(elementType.name, elementId)
        ) { it.toOsmQuestEntry() }

    suspend fun getAllInBBox(bounds: BoundingBox, questTypes: Collection<String>? = null): List<OsmQuestDaoEntry> {
    ...
    suspend fun getAllIdsInBBox(bounds: BoundingBox): List<Long> {
